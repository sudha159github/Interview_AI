    ...
    ILogger<GeminiInterviewReportGenerator> logger) : IInterviewReportGenerator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var request = new GeminiRequest(
            SystemInstruction: new GeminiContent(null, [new GeminiPart(GeminiPrompts.SystemInstruction)]),
            Contents:
            [
                new GeminiContent("user",
                [
                    new GeminiPart(GeminiPrompts.BuildUserPrompt(jobDescription, resumeText, selfDescription))
                ])
            ],
            GenerationConfig: new GeminiGenerationConfig("application/json", GeminiPrompts.ResponseSchema));

        var started = Stopwatch.GetTimestamp();

        // 2. Call Gemini (timeouts / retries / circuit breaker come from the resilience handler)
        GeminiResponse? body;
        try
        {
            using var response = await httpClient.PostAsJsonAsync(
                $"models/{_options.Model}:generateContent", request, JsonOptions, cancellationToken);
            ...
            body = await response.Content.ReadFromJsonAsync<GeminiResponse>(JsonOptions, cancellationToken);
        }
        catch (Exception ex) when (ex is HttpRequestException or TimeoutRejectedException or BrokenCircuitException)
        {
            logger.LogWarning(ex, "Gemini call failed for model {Model}", _options.Model);
            throw new AiGenerationException("The AI service is unavailable right now.");
        }

        // 3. Extract the JSON text (skip "thought" parts from thinking models)
        var candidate = body?.Candidates?.FirstOrDefault();
        var text = string.Concat(
            candidate?.Content?.Parts?.Where(p => p.Thought != true).Select(p => p.Text) ?? []);

        if (string.IsNullOrWhiteSpace(text))
        {
            logger.LogWarning(
                "Gemini returned no text. Finish reason: {FinishReason}. Block reason: {BlockReason}",
                candidate?.FinishReason, body?.PromptFeedback?.BlockReason);
        ...
        try
        {
            result = JsonSerializer.Deserialize<AiInterviewReportResult>(text, JsonOptions);
        }
        catch (JsonException ex)
        {
            logger.LogWarning(ex, "Gemini returned JSON that doesn't match the expected shape");
        ...
        // 6. Log timing and usage only — never the prompt or the output (personal data)
        logger.LogInformation(
            "Gemini generated a report in {ElapsedMs} ms using {Model} ({PromptTokens} prompt tokens, {OutputTokens} output tokens)",
